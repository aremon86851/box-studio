...
const ServicesDetails = () => {
    ...
    useEffect(() => {
        fetch(`https://assignment-11-server-teal-eight.vercel.app/reviews/${_id}`)
            .then(res => res.json())
            .then(data => setRevInfo(data))
    }, [_id])
    return (
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-5 mx-5'>
            <div className="card my-10 lg:col-span-2">
                <PhotoProvider>
                    <PhotoView src={img}>
                ...
                <div className="card-body">
                    <h2 className="card-title text-xl md:text-3xl">Name : <span className='italic font-bold text-red-700'>{title}</span></h2>
